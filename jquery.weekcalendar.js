...
(function($) {
  ...
  $.widget('ui.weekCalendar', (function() {
    ...
    return {
      ...
      /*
        * render sidabar with datepicker
      */
      _renderSidebar: function(){
        var sidebar = '<div class=\"calendar-sidebar\"><div id=\"calendar-sidebar-date\" class=\"calendar-sidebar-container\"><div></div>';
        $(sidebar).appendTo(this.element);

        var self = this;
        var dateInput = $('#calendar-sidebar-date').datepicker();

        dateInput.bind('change', function() {
          var newDate = this.value;


          self.gotoDate(newDate);
        });
      },
      ...
      _renderCalendarButtons: function($calendarContainer) {
        ...
        if (options.buttons) {
            ...
            calendarNavHtml += '<div class=\"calendar wrapper\">';
            // calendarNavHtml += '<div class=\"calendar-btn\"></div>';
            ...
            calendarNavHtml += '</div>';
            calendarNavHtml += '</div>';
            calendarNavHtml += '<div class=\"ui-widget-sidebar\">';
            calendarNavHtml += '</div>';
            ...
            // $calendarContainer.find('.wc-nav .calendar-btn')
            //   .button({
            //     icons: {primary: 'ui-icon-home'}})
            //   .click(function() {
            //         self.today();
  ...
  TODO    if total height for an hour is less than 11px, there is a display problem.
      ...
      _adjustForEventCollisions: function($weekDay, $calEvent, newCalEvent, oldCalEvent, maintainEventDuration) {
          ...
          $weekDay.find('.wc-cal-event').not($calEvent).each(function() {
            ...
            //return if collides with cloned event
            if (!currentCalEvent) {
              return;
            }
        ...
        $calEvent.draggable({
          ...
          drag: function(event, ui) {
            if (ui.originalPosition.top !== ui.position.top) {
              var diff = ((ui.originalPosition.top - ui.position.top) / options.timeslotHeight) * ((60 / options.timeslotsPerHour * -1) * MILLIS_IN_DAY/24/60);
              var start = new Date();
              var end = new Date();
              start.setTime(calEvent.start.getTime() + diff);
              end.setTime(calEvent.end.getTime() + diff);

              var newTime = calendar.weekCalendar('formatTime', start) + options.timeSeparator + calendar.weekCalendar('formatTime', end);

              $(ui.helper).find('.wc-time').text(newTime);
            }
          },
